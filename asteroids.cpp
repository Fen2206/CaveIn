...
float resolution_scale(Image *img)
{
    if (!img || img->width == 0 || img->height == 0)
        return 1.0f;
    float sx = (float)gl.xres / (float)img->width;
    float sy = (float)gl.yres / (float)img->height;
    return (sx > sy) ? sx : sy;
}
...
void render()
{
	...
	gl.background.show(gl.xres/2, gl.xres/2, gl.yres/2, 0.0f);

	//gl.background.show(gl.xres, gl.xres/2, gl.yres/2, 0.0f);
	...
	r.left = 10;
	r.center = 0;
	//ggprint(&r, 9, 16, 0x00ff0000, "3350 - Asteroids");
	//ggprint(&r, 9, 16, 0x00ffff00, "n bullets: %i", g.nbullets);
	//ggprint(&r, 9, 16, 0x00ffff00, "n asteroids: %i", g.nasteroids);
	//-------------------------------------------------------------------------
	//Draw the ship
